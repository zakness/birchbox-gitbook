...
[`Collapse`](https://github.com/zakness/birchbox-gitbook/tree/1ad9356b440d8ffd191f6222475ef6f0c15444b0/src/components/Collapse/Collapse/index.js) is an [adapted](../enhancers/adapter.md) collapsible component with a highly-customizable header and content. If you have a group of `Collapse`s and want to ensure only one is expanded at a time, wrap them in [`Accordion`](collapse.md#accordion).
...
| collapseKey | `string` | Optional. Inside a controlled `Accordion`, `Collapse`s are collapsed unless their `collapseKey` is equal to the `Accordion`’s `expandedKey`. See [`Accordion` docs](collapse.md#accordion). |  |
